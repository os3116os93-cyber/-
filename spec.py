...
def load_data(file_name):
    ...
    for f in all_files:
        if f.replace(" ", "").lower() == file_name.replace(" ", "").lower():
...
# [4] 품질보증기준 전용 테이블 (괄호 줄바꿈 + 초록색 한 줄 로직 추가)
def render_qc_table(df):
    ...
    for r_idx in range(len(df)):
        ...
        for c_idx in range(len(df.columns)):
            ...
            # [수정] 빨간색 도형 요청: 괄호 내용을 다음 줄로 ( <br> 삽입 )
            if "(" in val and ")" in val:
                val = val.replace("(", "<br>(")
            
            if span > 0:
                # [수정] 초록색 도형 요청: 중복 병합된 셀(1, 2열)은 nowrap 적용하여 한 줄로
...
def main():
    ...
    with tab1:
        ...
        else: st.error("파일을 찾을 수 없습니다.")

    # --- TAB 2: 품질 보증 기준 (신규 줄바꿈/한줄 로직 적용) ---
    with tab2:
        df2 = load_data("11.xlsx")
        if df2 is not None:
