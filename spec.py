...
# ── 핵심 수정: "collapsed" → "expanded"
# 이렇게 해야 사이드바 내부 상태와 CSS가 충돌하지 않음
st.set_page_config(
...
def show_home():
    # ── 홈 화면: 사이드바를 CSS로 숨기는 대신,
    #    사이드바에 아무 위젯도 렌더링하지 않으면
    #    Streamlit이 자동으로 사이드바를 닫힌 상태로 표시함.
    #    initial_sidebar_state="expanded" 상태에서도
    #    내용이 없으면 자동으로 접혀 보임.
    #    → CSS 충돌 없이 깔끔하게 처리됨.
...
if st.session_state.page == "home":
    # 홈: 사이드바에 아무것도 렌더링하지 않음
    # → Streamlit이 자동으로 사이드바를 숨겨줌 (CSS 조작 불필요)
    show_home()

elif st.session_state.page == "coil":
    # coil: 사이드바에 홈 버튼만 표시
    if st.sidebar.button("← 홈으로 돌아가기", key="coil_home_btn"):
        st.session_state.page = "home"
        st.rerun()
    _render_home_btn()
    import app_coil
    ...
    # cutting: 사이드바는 app_cutting.main() 내부에서 렌더링
    import app_cutting
    app_cutting.run()
